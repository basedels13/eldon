                ...
                case 0:
                  ...
                  socket.on("room-update", (data)=>{
                    if(data.status){
                      ...
                      cx2.fillText('4人でドンジャラ　半荘戦',170,40);
                      cx2.fillText('スキル禁止　満貫ブロックあり',170,75);
...
function Setup(pvp=0){
  ...
  switch(LP[0]){
    ...
    if(pvp==1){
      ...
      for(var i=0; i<memberlist.length; i++){
        chara[i]=memberlist[i].chr
        MEMBER.push({id:memberlist[i].id,token:memberlist[i].token,name:memberlist[i].name,chr:memberlist[i].chr,score:LP[i],turnflag:0,pc:1});
      };
      //CPUを追加
      var A=MEMBER.length;
      if(A<4){
      ...
      //chara[i]=R;
      chara[i]=i;
          MEMBER.push({id:MEMBER.length,token:0,name:"CPU"+i,chr:chara[i],score:LP[i],turnflag:0,pc:0});
        };
      }
      function shuffleM(){
      ...
      console.log(MEMBER);
      socket.emit("setup_member", MEMBER);
    return true;
    };
  if(chara[0]==0){
  ...
  socket.on("member",(data)=>{
    ...
    chrtemp.splice(A,4);
    for(var i=1;i<5 ; i++){
    chara[i]=chrtemp[i-1].chr;
    }
    console.log(chara);
    parent=chrtemp.findIndex(value=>value.turnflag==1);
  }
    gamestate =1
    if( mute=="ON" ){
      Bgm.playMusic();
    ...
    clearTime = Date.now()
    timevalue =clearTime - timevalue;
    console.log('setup for pvp',timevalue)
    deckHandler(1);
  });
...
cx2.fillText("スキンを張る.", 635, 150);
cx2.fillText("(特に意味はない)", 635, 170);
}else if(p==2){
  var MS=Buff[player].filter(value=>value==2)
  if(!MS.length){MS=[]};
cx2.font = "bold 16px Arial";
